...
router.get('/friendsTopic', function(req, res, next) {
  ...
  if (req.session.userInfo) {
    ...
    let hot_sql = 'select * from ( ' 
      +'select * from topic where type = 3 order by created_at limit ?,?) ' 
          ...
          db.query(topic_sql,item.topic_id,(err,rst_hotTopic) => {
            rst_hotTopic.map(card => {
              ...
              db.query(commentCount_sql,item.topic_id,(err,comment_counts) => {
                ...
                item.friends_topic.comment_count = comment_counts[0]["count(1)"];
              })
              ...
              db.query(likeCount_sql,item.topic_id,(err,like_counts) => {
                ...
                item.friends_topic.like_count = like_counts[0]["count(1)"];
              })
    ...
    ' from `like` as l,topic as t where l.topic_id=t.topic_id and t.type = 2'+
    ' group by l.topic_id'+ 
